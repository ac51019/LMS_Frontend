  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      if (usersRes.success) {
        ...
        const instructors = usersRes.data.filter(u => u.role === "INSTRUCTOR");
  ...
  return (
    <>
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        {/* Instructors */}
        <li className="group bg-white/60 backdrop-blur-xl rounded-3xl p-8 flex items-center gap-6 shadow-lg hover:shadow-2xl border border-white/30 transition-transform duration-300 hover:-translate-y-2">
          <div className="w-20 h-20 flex items-center justify-center rounded-2xl bg-gradient-to-br from-purple-500 to-pink-600 text-white text-4xl shadow-md group-hover:scale-110 transition-transform duration-300">
            <i className="bx bxs-user-badge" />
          </div>
          <div>
            <h3 className="text-3xl font-bold text-slate-900">{instructorsCount}</h3>
            <p className="text-slate-600 text-lg">Total Instructors</p>
          </div>
        </li>
