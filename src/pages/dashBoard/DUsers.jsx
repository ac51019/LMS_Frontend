      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h3 className="text-3xl font-bold text-slate-800 tracking-tight">
            Users Management {roleFilter !== "ALL" && `- ${roleFilter}S`}
          </h3>
          <p className="text-slate-600 mt-2">
            Manage and view registered {roleFilter === "ALL" ? "users, instructors, and admins" : roleFilter.toLowerCase() + "s"}
          </p>
        </div>
        <div className="flex flex-wrap bg-white rounded-lg p-1 shadow-sm border border-gray-200">
          {["ALL", "USER", "INSTRUCTOR", "ADMIN"].map(role => (
            <button
              key={role}
