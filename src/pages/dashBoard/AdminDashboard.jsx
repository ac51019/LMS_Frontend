...
function AdminDashboard() {
  ...
  const handleLogin = async (e) => {
    ...
    if (result.success && (result.user.role === "ROLE_ADMIN" || result.user.role === "ROLE_SUPER_ADMIN")) {
      setIsAuthenticated(true);
      setError("");
    } else if (result.success && (result.user.role !== "ROLE_ADMIN" && result.user.role !== "ROLE_SUPER_ADMIN")) {
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50 relative">
      ...
      <section className="flex-1 min-w-0 flex flex-col h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-100 transition-all duration-300">
        <header className="flex items-center justify-between p-4 md:hidden bg-white shadow-sm">
          <h1 className="text-xl font-bold text-slate-800">LMS ADMIN</h1>
          <button onClick={() => setSidebarOpen(true)} className="p-2 text-slate-600 focus:outline-none">
            <i className="bx bx-menu text-3xl"></i>
          </button>
        </header>
        <main className="flex-1 p-4 md:p-8 font-poppins overflow-y-auto w-full">{renderContent()}</main>
      </section>
