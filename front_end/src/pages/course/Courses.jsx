...
const mockImages = [c1, c2, c3, c4, c5, c6];


const SkeletonCard = () => (
  <div className="bg-white rounded-2xl shadow-lg border-0 overflow-hidden animate-pulse">
...
function Courses() {
  ...
  useEffect(() => {
    if (coursesStatus === 'succeeded' || coursesStatus === 'failed') {
      if (coursesStatus === 'succeeded' && globalCourses) {
        const processedCourses = globalCourses.map((c, index) => ({
          ...c,
          p_link: (c.p_link && c.p_link.startsWith("http")) ? c.p_link : mockImages[index % 6],
          price: c.price ? formatCurrency(c.price) : formatCurrency(499)
        }));
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    {enrolled.includes(course.course_id) ? (
                      <button
                        onClick={(e) => { e.stopPropagation(); navigate("/learnings"); }}
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); enrollCourse(course.course_id); }}
