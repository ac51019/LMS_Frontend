...
function CourseModal({ isOpen, onClose, onSuccess, courseId = null, mode = "add" }) {
  ...
  const fetchInstructors = async () => {
    try {
      ...
      if (result.success) {
        setInstructors(result.data.filter(u => u.role === "INSTRUCTOR" || u.role === "ADMIN"));
      }
    } catch {
      // suppress error
    }
            ...
            <Form.Item noStyle shouldUpdate>
              {() => (
                <select
                  ...
                  {...form.getFieldProps("instructor")}
                >
                  <option value="" disabled>Select an instructor</option>
                  {instructors.map((inst) => (
                    <option key={inst.id} value={inst.username}>
                      {inst.username} ({inst.email})
                    </option>
                  ))}
                  {/* Allow entering custom if array is empty (fallback) */}
                  {instructors.length === 0 && <option value="Custom">Custom Instructor</option>}
                </select>
              )}
            </Form.Item>
          </Form.Item>
